...
一套面板，三种运行形态（单机站点 / 静态托管 / 书签注入），核心能力一致：

- 凭据来源按形态而定：单机形态由本地进程走 CAS 协议登录取得；
  注入形态则从宿主页面的 `localStorage` / `sessionStorage` 同源读取
- 拉取全部 **142 栋宿舍楼**坐标，按你所在楼栋计算围栏
- **两种打卡模式**：人在宿舍时用「实时定位」直接提交手机定位；
  不在校时用「虚拟落点」在围栏内生成一个合规坐标
...
| 绿 / 橙 / 灰圆点 | **邻近楼栋**（围栏或范围外扩 100m 内的）：落在落点范围内 / 围栏内但超范围 / 已在围栏外。**点击即设为圆心**，悬停显示楼栋名与距离 |
| 绿色图钉 | 本次生成的落点，可拖拽微调 |
| 灰色小点 | 历史落点，最多 25 个，只读不可点 |
...
- 注入形态的面板只在 `*.csu.edu.cn` 域内工作（要读宿主凭据，同源要求）；
  单机形态没有这个限制
